...
describe('ChargingService', () => {
  ...
  describe('start', () => {
    ...
    it('should throw NotFoundException if charger not found', async () => {
      ...
      await expect(service.start('user-1', startDto)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw ConflictException if user already has active session', async () => {
      ...
      prisma.chargingSession.findFirst.mockResolvedValueOnce({
        id: 'existing',
        status: 'ACTIVE',
      });
      await expect(service.start('user-1', startDto)).rejects.toThrow(
  ...
  describe('stop', () => {
    it('should stop and calculate cost', async () => {
      prisma.chargingSession.findUnique.mockResolvedValue({
        ...
        startTime: new Date(),
        station: {
          tariff: { id: 'tariff-1', isActive: true, pricePerKwh: 2.5 },
        },
      });
      prisma.chargingSession.update.mockResolvedValue({
        ...
        station: { id: 'station-1', name: 'Station 1' },
        charger: { id: 'charger-1', serialNumber: 'SN-001' },
        connector: { id: 'conn-1' },
      });

      const result = await service.stop('session-1', 'user-1', {
        energyKwh: 32.5,
      });
    ...
    it('should throw NotFoundException if session not found', async () => {
      ...
      await expect(
        service.stop('nonexistent', 'user-1', { energyKwh: 10 }),
  ...
  describe('updateEnergy', () => {
    it('should update energy and emit real-time event', async () => {
      prisma.chargingSession.findUnique.mockResolvedValue({
        ...
        startTime: new Date(),
        energyKwh: 10,
        station: { tariff: { isActive: true, pricePerKwh: 2.5 } },
      });
      prisma.chargingSession.update.mockResolvedValue({
        id: 'session-1',
        userId: 'user-1',
      ...
      const result = await service.updateEnergy('session-1', 'user-1', {
        energyKwh: 25.5,
      });
